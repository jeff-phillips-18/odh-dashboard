...
const ConnectedInstalledApplications: React.FC<ConnectedInstalledApplicationsProps> = ({ components, getComponents, componentsLoading, componentsError }) => {
        ...
        <PageSection className="odh-installed-apps__error">
          <EmptyState variant={EmptyStateVariant.full}>
            ...
            <EmptyStateBody className="odh-installed-apps__error-body">
              <div>
                <code className="odh-installed-apps__display-error">{JSON.stringify(componentsError, null, 2)}</code>
              </div>
            </EmptyStateBody>
          </EmptyState>
        </PageSection>
    ...
    <>
      <PageSection className="odh-installed-apps__heading" variant={PageSectionVariants.light}>
        ...
        <ToggleGroup aria-label="supported applications">
          <ToggleGroupItem text="Red Hat supported" isSelected={supportType === 'redhat'} onChange={() => setSupportType('redhat')} />
          <ToggleGroupItem text="Third-party supported" isSelected={supportType === 'other'} onChange={() => setSupportType('other')} />
        </ToggleGroup>
      </PageSection>
      {buildComponentList()}
    </>
